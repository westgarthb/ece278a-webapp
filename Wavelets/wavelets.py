...
def continous():
    ...
    st.subheader('Continuous Wavelets')

    st.write('Definition of contious Wavelets')

    # Formula for CWT
    st.latex(r'''T\left(a, b\right) = ''')
    #st.latex(r'''g\left(i, j\right) = \sum_{k,l} f(k, l)h(i - rk, j - rl)''')

# TODO: Discrete
def discrete():
    st.subheader('Discrete Wavelets')

    st.write('Definition of Discrete Wavelets')

    # Formula for CWT
    st.latex(r'''T\left(m n\right) = ''')
    #st.latex(r'''g\left(i, j\right) = \sum_{k,l} f(k, l)h(i - rk, j - rl)''')
...
def display_wavelets():
    ...
    # st.write('A wavelet family is a collection of functions generated by changing frequency parameter \"a\" and shift frequency \"b\" explicitly in the form:')
    # st.latex(r'''\Psi_{a,b}(x) = \frac{1}{\sqrt{a}}\Psi^*\frac{(x-b)}{a}dt''')
    # st.subheader("Example")
    # fig_a_index = st.slider("Change \"a\" parameter", 0, 9, 5)
    ...
    # st.image(wav_a_fig)
    # fig_b_index = st.slider("Change \"b\" parameter", 0, 9, 5)
    ...
    # st.write('The Continuous Wavelet Transform is defined by:')
